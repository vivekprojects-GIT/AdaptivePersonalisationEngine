...
<template>
  <div class="max-w-4xl mx-auto p-4 space-y-4">
    ...
    <div v-if="error" class="rounded-lg border border-rose-500/40 bg-rose-500/5 px-3 py-2 text-sm text-rose-600 dark:text-rose-400">
      {{ error }}
    </div>
            ...
            <tbody>
              <tr v-for="(c, i) in result.verification.checks" :key="i" class="border-t border-border/60">
                <td class="px-3 py-1.5">
                  <span :class="c.ok ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'">
                    {{ c.ok ? '✓' : '✗' }}
                  </span>
                </td>
